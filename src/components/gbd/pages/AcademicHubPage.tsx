...
import { Trash2, Plus, ChevronDown, ChevronUp, Edit, Calculator, TrendingUp, BookOpen } from 'lucide-react';
    ...
    <div className="page-enter max-w-[1200px] mx-auto">
      <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
        ...
        <div className="tab-group">
          {[
            { key: 'tracker', icon: BookOpen, label: 'TRACKER' },
            { key: 'calculator', icon: Calculator, label: 'CALCULATOR' },
            { key: 'simulation', icon: TrendingUp, label: 'SIMULATION' },
          ].map(t => (
            <button key={t.key} className={`tab-item ${hubTab === t.key ? 'active' : ''}`} onClick={() => setHubTab(t.key)}>
              {t.label}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="glass-card-accent !p-4">
          ...
          <div className={`text-2xl font-bold ${requiredGPA > 4 ? 'text-destructive' : 'text-foreground'}`}>
            {requiredGPA > 4 ? '4.00+' : requiredGPA.toFixed(2)}
          </div>
      ...
      {hubTab === 'tracker' && (
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="btn-green" onClick={() => setShowAddSemester(true)}>
              <Plus className="w-4 h-4 inline-block mr-1" /> Add Semester
            </button>
          </div>
          ...
          {showAddSemester && (
            <div className="glass-card mb-4 flex gap-3 items-end">
              <div className="flex-1">
                ...
                <input type="text" id="sem-name-input" className="input-simple" placeholder="e.g. Fall 2025" />
              </div>
                ...
                <div className="flex justify-between items-center cursor-pointer" onClick={() => setExpandedSem(isExpanded ? null : s.id)}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-xs text-muted-foreground">{s.courses.length} course{s.courses.length !== 1 ? 's' : ''}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-primary font-bold text-lg">{s.courses.length > 0 ? gpa.toFixed(2) : '—'}</span>
                    <button className="icon-btn !w-7 !h-7 !text-destructive" onClick={(e) => { e.stopPropagation(); deleteSemester(s.id); }}>
                  ...
                  <div className="mt-4 border-t pt-4" style={{ borderColor: 'hsl(var(--border))' }}>
                    {s.courses.length > 0 && (
                      <div className="space-y-2 mb-3">
                        <div className="grid grid-cols-[1fr_80px_80px_40px] gap-2 text-[0.65rem] font-semibold tracking-widest text-muted-foreground px-1">
                        ...
                        {s.courses.map((c: any) => (
                          <div key={c.id} className="grid grid-cols-[1fr_80px_80px_40px] gap-2 items-center p-2 rounded-lg" style={{ background: 'hsl(var(--bg-input))' }}>
                            ...
                            <button className="icon-btn !w-6 !h-6 !text-destructive" onClick={() => deleteCourse(s.id, c.id)}>
                    ...
                    {showAddCourse === s.id ? (
                      <div className="grid grid-cols-[1fr_100px_80px] gap-2 items-end mt-2">
                        <div>
                          ...
                          <input type="text" id={`course-name-${s.id}`} className="input-simple" placeholder="e.g. Calculus I" />
                        </div>
                        <div>
                          <label className="form-label">GRADE</label>
                          <select id={`course-grade-${s.id}`} className="input-simple" defaultValue="A">
                            {GRADE_OPTIONS.map(g => <option key={g} value={g}>{g} ({GRADE_MAP[g].toFixed(2)})</option>)}
                          </select>
                        </div>
                        <div>
                          <label className="form-label">CREDITS</label>
                          <input type="number" id={`course-credits-${s.id}`} className="input-simple" defaultValue={3} min={1} max={10} />
                        </div>
                      ...
                      <button className="btn-outline w-full mt-1" onClick={() => setShowAddCourse(s.id)}>
                        <Plus className="w-3.5 h-3.5 inline-block mr-1" /> Add Course
                      </button>
                    )}
                  </div>
                )}
        ...
        <div>
          <div className="glass-card mb-6">
            ...
            <div className="space-y-2 mb-4">
              <div className="grid grid-cols-[1fr_120px_80px_40px] gap-2 text-[0.65rem] font-semibold tracking-widest text-muted-foreground px-1">
              ...
              {calcCourses.map((row, i) => (
                <div key={i} className="grid grid-cols-[1fr_120px_80px_40px] gap-2 items-center">
                  <input type="text" className="input-simple" placeholder="Course name"
                    value={row.name} onChange={e => updateCalcRow(i, 'name', e.target.value)} />
                  <select className="input-simple" value={row.grade} onChange={e => updateCalcRow(i, 'grade', e.target.value)}>
                    {GRADE_OPTIONS.map(g => <option key={g} value={g}>{g}</option>)}
                  </select>
                  <input type="number" className="input-simple" min={1} max={10}
                    value={row.credits} onChange={e => updateCalcRow(i, 'credits', parseInt(e.target.value) || 1)} />
                  <button className="icon-btn !w-8 !h-8 !text-destructive" onClick={() => removeCalcRow(i)} disabled={calcCourses.length <= 1}>
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
                </div>
              ))}
            ...
            <button className="btn-outline mb-4" onClick={addCalcRow}>
              <Plus className="w-3.5 h-3.5 inline-block mr-1" /> Add Course
            </button>
          ...
          <div className="glass-card mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <input type="number" className="input-simple" step={0.01} min={0} max={4}
                  value={simTargetCGPA} onChange={e => setSimTargetCGPA(parseFloat(e.target.value) || 0)} />
              </div>
              <div>
                <label className="form-label">REMAINING CREDITS TO COMPLETE</label>
                <input type="number" className="input-simple" min={1} max={200}
                  value={simCredits} onChange={e => setSimCredits(parseInt(e.target.value) || 1)} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div className="p-4 rounded-lg text-center" style={{ background: simRequiredGPA() > 4 ? 'hsl(var(--destructive) / 0.1)' : 'hsl(var(--accent-dim))' }}>
                <div className="text-[0.65rem] font-semibold tracking-widest text-muted-foreground mb-1">REQUIRED GPA</div>
                <div className={`text-2xl font-bold ${simRequiredGPA() > 4 ? 'text-destructive' : 'text-primary'}`}>
                  {simRequiredGPA() > 4 ? '> 4.00' : simRequiredGPA().toFixed(2)}
                </div>
                <div className="text-xs text-muted-foreground">
                  {simRequiredGPA() > 4 ? 'Not achievable' : simRequiredGPA() <= 0 ? 'Already achieved!' : `across ${simCredits} credits`}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              {[4.00, 3.70, 3.50, 3.30, 3.00, 2.50].map(gpa => {
                const projected = totalCredits + simCredits > 0
                  ? (totalPoints + gpa * simCredits) / (totalCredits + simCredits)
                  : 0;
                ...
                return (
                  <div key={gpa} className="flex items-center justify-between p-3 rounded-lg" style={{ background: 'hsl(var(--bg-input))' }}>
                    <span className="text-sm text-foreground">
                      If you maintain <span className="font-bold text-primary">{gpa.toFixed(2)}</span> GPA for {simCredits} credits
                    </span>
                    <div className="flex items-center gap-3">
                      <span className={`text-sm font-bold ${meetsTarget ? 'text-primary' : 'text-destructive'}`}>
                        {projected.toFixed(2)} CGPA
                      </span>
                      <span className={`text-xs px-2 py-0.5 rounded font-semibold ${meetsTarget ? 'bg-primary/10 text-primary' : 'bg-destructive/10 text-destructive'}`}>
