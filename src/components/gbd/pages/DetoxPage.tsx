    ...
    return (
      <div className="page-enter max-w-[800px] mx-auto text-center">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-xl font-bold text-primary">🛡️ Detox</h2>
            <p className="text-xs text-muted-foreground">DISTRACTIONS BLOCKED</p>
          </div>
          <div className="flex items-center gap-1 text-4xl font-mono font-bold text-foreground">
            <span>{m}</span><span className="text-muted-foreground">:</span><span>{s}</span>
          </div>
        ...
        <div className="mb-8">
          <TreeSVG stage={stage} />
          <div className="mt-3"><span className="text-xs font-bold px-3 py-1 rounded-full" style={{ background: 'hsl(var(--accent-dim))', color: 'hsl(var(--primary))' }}>STAGE {stage}/5</span></div>
        </div>

        <div className="glass-card !p-6 mb-6">
          <Shield className="w-16 h-16 text-primary mx-auto mb-4" />
          <h2 className="text-lg font-bold text-foreground mb-2">Deep Focus Active</h2>
          <p className="text-sm text-muted-foreground">Stay focused. All distractions are blocked.</p>
        </div>

        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="glass-card !p-4">
            <div className="text-2xl font-bold text-primary">{Math.min(100, parseFloat(progress))}%</div>
            <div className="text-[0.65rem] font-semibold tracking-widest text-muted-foreground">FOCUS LEVEL</div>
          </div>
          <div className="glass-card !p-4">
            <div className="text-2xl font-bold text-foreground">5</div>
            <div className="text-[0.65rem] font-semibold tracking-widest text-muted-foreground">SITES BLOCKED</div>
          </div>
        </div>

        <div className="xp-bar !h-3">
          <div className="xp-bar-fill" style={{ width: `${Math.min(100, parseFloat(progress))}%` }} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="page-enter max-w-[1200px] mx-auto">
      <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Detox</h1>
          <p className="text-muted-foreground text-sm">Lock in. Eliminate distractions. Grow your focus tree.</p>
        </div>
        <button className="btn-green flex items-center gap-2" onClick={() => setShowSetup(true)}>
        ...
        <div className="glass-card mb-6">
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2 flex-wrap">
              {[25, 50, 90].map(d => (
                <button key={d} className={`dur-btn ${duration === d ? 'active' : ''}`} onClick={() => setDuration(d)}>{d}m</button>
              ))}
              <input type="number" className="input-simple w-24" placeholder="Custom" min={1} max={240}
                onChange={e => { const v = parseInt(e.target.value); if (v > 0) setDuration(v); }} />
            </div>
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                { id: 'white', label: 'WHITE', icon: Volume2 },
                { id: 'lofi', label: 'LOFI', icon: Volume2 },
              ].map(s => {
                const Icon = s.icon;
                return (
                  <button key={s.id} className={`sound-btn ${sound === s.id ? 'active' : ''}`} onClick={() => setSound(s.id)}>
          ...
          <div className="flex gap-3 mt-5">
            <button className="btn-green flex-1 flex items-center justify-center gap-2" onClick={startFocus}>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="glass-card !p-4 text-center">
          ...
          <div className="text-[0.65rem] font-semibold tracking-widest text-muted-foreground">TOTAL SESSIONS</div>
        </div>
        <div className="glass-card !p-4 text-center">
          <div className="text-2xl font-bold" style={{ color: 'hsl(var(--purple))' }}>{(totalMin / 60).toFixed(1)}h</div>
          <div className="text-[0.65rem] font-semibold tracking-widest text-muted-foreground">FOCUS TIME</div>
        </div>
        <div className="glass-card !p-4 text-center">
          <div className="text-2xl font-bold" style={{ color: 'hsl(var(--warning))' }}>{stage}/5</div>
          <div className="text-[0.65rem] font-semibold tracking-widest text-muted-foreground">TREE STAGE</div>
        </div>
      </div>
      ...
      <div className="glass-card text-center !p-8 mb-6">
        <TreeSVG stage={stage} />
        <div className="mt-3"><span className="text-xs font-bold px-3 py-1 rounded-full" style={{ background: 'hsl(var(--accent-dim))', color: 'hsl(var(--primary))' }}>STAGE {stage}/5</span></div>
        <p className="text-sm text-muted-foreground mt-2">Keep focusing to grow your tree 🌳</p>
      </div>
      ...
      <div className="glass-card">
        ...
        {sessions.length === 0 ? (
          <p className="text-sm text-muted-foreground">No focus sessions yet. Start your first one!</p>
        ) : sessions.slice().reverse().slice(0, 10).map((s: any) => (
