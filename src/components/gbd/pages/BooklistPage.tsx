...
const BooklistPage = ({ navigateTo }: BooklistPageProps) => {
  ...
  const STATUS_TABS = [
    { id: 'reading', label: t('booklist.reading'), icon: BookOpen, color: 'hsl(var(--primary))' },
    { id: 'want-to-read', label: t('booklist.want_to_read'), icon: Bookmark, color: 'hsl(var(--warning))' },
    { id: 'finished', label: t('booklist.finished_label'), icon: CheckCircle, color: 'hsl(var(--green))' },
  ];
  ...
  const openAddModal = (status?: string) => { setEditingId(null); setNewRating(0); setNewGenre('Fiction'); setNewStatus(status || activeTab); setShowModal(true); };
  const openEditModal = (book: any) => { setEditingId(book.id); setNewRating(book.rating || 0); setNewGenre(book.genre || 'Fiction'); setNewStatus(book.status || 'reading'); setShowModal(true); };
  ...
  const saveBook = async () => {
    ...
    if (!title) { await showDialog({ title: 'Missing Info', message: 'Please enter a book title.', type: 'alert' }); return; }
    ...
    <div className="page-enter">
      <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">{t('booklist.title')}</h1>
            <p className="text-muted-foreground text-sm">{t('booklist.subtitle')}</p>
          </div>
        </div>
        <button className="btn-green" onClick={() => openAddModal()}><Plus className="w-4 h-4" /> {t('booklist.add_book')}</button>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div className="glass-card !p-4 text-center">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center mx-auto mb-2" style={{ background: 'hsl(var(--purple) / 0.12)', color: 'hsl(var(--purple))' }}><Book className="w-4.5 h-4.5" /></div>
          <div className="text-xl font-bold text-foreground">{totalBooks}</div>
          <div className="text-[0.7rem] font-semibold tracking-widest text-muted-foreground">{t('booklist.total_books')}</div>
        </div>
        <div className="glass-card !p-4 text-center">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center mx-auto mb-2" style={{ background: 'hsl(var(--primary) / 0.12)', color: 'hsl(var(--primary))' }}><BookOpen className="w-4.5 h-4.5" /></div>
          <div className="text-xl font-bold text-primary">{readingCount}</div>
          <div className="text-[0.7rem] font-semibold tracking-widest text-muted-foreground">{t('booklist.reading_now')}</div>
        </div>
        <div className="glass-card !p-4 text-center">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center mx-auto mb-2" style={{ background: 'hsl(var(--green) / 0.12)', color: 'hsl(var(--green))' }}><CheckCircle className="w-4.5 h-4.5" /></div>
          <div className="text-xl font-bold" style={{ color: 'hsl(var(--green))' }}>{finishedCount}</div>
          <div className="text-[0.7rem] font-semibold tracking-widest text-muted-foreground">{t('booklist.finished')}</div>
        </div>
        <div className="glass-card !p-4 text-center">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center mx-auto mb-2" style={{ background: 'hsl(var(--warning) / 0.12)', color: 'hsl(var(--warning))' }}><Star className="w-4.5 h-4.5" /></div>
          <div className="text-xl font-bold" style={{ color: 'hsl(var(--warning))' }}>{totalPages}</div>
          <div className="text-[0.7rem] font-semibold tracking-widest text-muted-foreground">{t('booklist.pages_read')}</div>
        </div>
      </div>
        ...
        {STATUS_TABS.map(tab => {
          ...
          return (
            <button key={tab.id}
              ...
              <span className="text-xs px-1.5 py-0.5 rounded-full" style={{ background: activeTab === tab.id ? `${tab.color}20` : 'hsl(var(--bg-input))' }}>{count}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6">
        <div>
          <div className="search-input-wrap mb-4">
            ...
            <input type="text" placeholder={t('booklist.search_books')} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
          ...
          <div className="space-y-2 max-h-[55vh] overflow-y-auto pr-1">
            {filtered.length === 0 ? (
              <div className="glass-card empty-state !p-10">
                ...
                <p className="text-sm text-muted-foreground">{searchQuery ? t('booklist.no_books_search') : `No books in "${STATUS_TABS.find(t => t.id === activeTab)?.label}"`}</p>
                <button className="btn-outline mt-3 !text-xs" onClick={() => openAddModal()}>+ {t('booklist.add_book')}</button>
              </div>
            ) : filtered.map((book: any) => (
              <div key={book.id} onClick={() => setSelectedBookId(book.id)}
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                      <span className="text-[0.65rem] font-semibold px-2 py-0.5 rounded" style={{ background: 'hsl(var(--purple) / 0.1)', color: 'hsl(var(--purple))' }}>{book.genre || 'General'}</span>
                    ...
                    {book.pages > 0 && book.status === 'reading' && (
                      <div className="mt-2">
                        <div className="xp-bar !h-1.5"><div className="xp-bar-fill" style={{ width: `${Math.min(100, Math.round(((book.currentPage || 0) / book.pages) * 100))}%` }} /></div>
        ...
        <div className="glass-card min-h-[400px]">
          {selectedBook ? (
            <div>
              <div className="flex justify-between items-start mb-5 gap-3 flex-wrap">
                ...
                <div className="flex gap-2 shrink-0">
                  <button className="btn-outline !py-1.5 !px-3 !text-xs" onClick={() => openEditModal(selectedBook)}><Edit className="w-3 h-3 inline-block mr-1" /> {t('common.edit')}</button>
                  <button className="btn-outline !py-1.5 !px-3 !text-xs !text-destructive !border-destructive/30" onClick={() => deleteBook(selectedBook.id)}><Trash2 className="w-3 h-3 inline-block mr-1" /> {t('common.remove')}</button>
                </div>
              </div>

              <div className="flex items-center gap-3 mb-5 flex-wrap">
                <span className="text-[0.65rem] font-semibold px-2.5 py-1 rounded" style={{ background: 'hsl(var(--purple) / 0.1)', color: 'hsl(var(--purple))' }}>{selectedBook.genre || 'General'}</span>
                {selectedBook.pages > 0 && <span className="text-xs text-muted-foreground">{selectedBook.currentPage || 0} / {selectedBook.pages} pages</span>}
                <span className="text-xs text-muted-foreground">Added {formatDate(selectedBook.addedAt)}</span>
              </div>

              {selectedBook.pages > 0 && (
                <div className="mb-5">
                  <label className="form-label">{t('booklist.reading_progress')}</label>
                  <div className="flex items-center gap-3 mb-2">
                    <div className="flex-1"><div className="xp-bar !h-3"><div className="xp-bar-fill" style={{ width: `${Math.min(100, Math.round(((selectedBook.currentPage || 0) / selectedBook.pages) * 100))}%` }} /></div></div>
                  ...
                  <div className="flex items-center gap-2">
                    <label className="text-[0.65rem] font-semibold text-muted-foreground shrink-0">{t('booklist.page')}</label>
                    <input type="number" className="input-simple !py-1.5 !px-3 !text-sm w-20" min={0} max={selectedBook.pages} value={selectedBook.currentPage || 0} onChange={e => updateProgress(selectedBook.id, parseInt(e.target.value) || 0)} />
                    <span className="text-xs text-muted-foreground">of {selectedBook.pages}</span>
                    {selectedBook.currentPage < selectedBook.pages && <button className="btn-outline !py-1 !px-2.5 !text-[0.65rem]" onClick={() => updateProgress(selectedBook.id, (selectedBook.currentPage || 0) + 1)}>+1</button>}
                    {selectedBook.pages > 10 && selectedBook.currentPage < selectedBook.pages && <button className="btn-outline !py-1 !px-2.5 !text-[0.65rem]" onClick={() => updateProgress(selectedBook.id, Math.min(selectedBook.pages, (selectedBook.currentPage || 0) + 10))}>+10</button>}
                  </div>
                </div>
              )}
              ...
              {selectedBook.rating > 0 && (
                <div className="mb-5">
                  <label className="form-label">{t('booklist.your_rating')}</label>
                  <StarRating rating={selectedBook.rating} onChange={(r) => { Storage.updateBook(selectedBook.id, { rating: r }); refresh(); }} />
                </div>
              )}

              <div className="mb-5">
                <label className="form-label">{t('booklist.status')}</label>
              ...
              {selectedBook.notes && (
                <div>
                  <label className="form-label">{t('booklist.notes')}</label>
            ...
            <div className="empty-state h-full flex flex-col items-center justify-center gap-3 min-h-[350px]">
              ...
              <p className="text-muted-foreground">{books.length === 0 ? t('booklist.empty_shelf') : t('booklist.select_book')}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => { setShowModal(false); setEditingId(null); }}>
          <div className="modal-card !max-w-lg" onClick={e => e.stopPropagation()}>
            <h2 className="text-lg font-bold text-foreground mb-4">{editingId ? t('booklist.edit_book') : t('booklist.add_book')}</h2>
            <div className="space-y-3">
              <div><label className="form-label">{t('booklist.title_label')}</label><input type="text" id="book-title" className="input-simple" placeholder="Book title" defaultValue={editingBook?.title || ''} /></div>
              <div><label className="form-label">{t('booklist.author')}</label><input type="text" id="book-author" className="input-simple" placeholder="Author name" defaultValue={editingBook?.author || ''} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="form-label">{t('booklist.genre')}</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="form-label">{t('booklist.total_pages')}</label><input type="number" id="book-pages" className="input-simple" placeholder="0" min={0} defaultValue={editingBook?.pages || ''} /></div>
                <div><label className="form-label">{t('booklist.current_page')}</label><input type="number" id="book-current-page" className="input-simple" placeholder="0" min={0} defaultValue={editingBook?.currentPage || ''} /></div>
              </div>
              <div><label className="form-label">{t('booklist.rating')}</label><StarRating rating={newRating} onChange={setNewRating} /></div>
              <div><label className="form-label">{t('booklist.notes')}</label><textarea id="book-notes" className="input-simple min-h-[80px] resize-y" placeholder="Your thoughts..." defaultValue={editingBook?.notes || ''} /></div>
            </div>
            <div className="flex gap-3 mt-5">
              <button className="btn-outline flex-1" onClick={() => { setShowModal(false); setEditingId(null); }}>{t('common.cancel')}</button>
              <button className="btn-green flex-1" onClick={saveBook}>{editingId ? t('common.update') : t('booklist.add_book')}</button>
            </div>
          </div>
        </div>
      )}
